...
function eventStore_toneoff = triggered_cam_changes_toneoff(folder, x1, x2, y1, y2, ploton)
...
eventStore_toneoff = cell(dims(2),1);

for n = 1:dims(2)
    
    disp(['Working on tone off #' int2str(n)]);
...
if(ploton)
    ...
    eventStore_toneoff{n}.horzTraceRaw = horzTrace;
    eventStore_toneoff{n}.vertTraceRaw = vertTrace';
    eventStore_toneoff{n}.horzTraceRef = horzTraceAlt;
    eventStore_toneoff{n}.vertTraceRef = vertTraceAlt;
    eventStore_toneoff{n}.camTimes     = stuff(:,n)+1;
    eventStore_toneoff{n}.folder       = folder;
    eventStore_toneoff{n}.horzChange   = horzChange;
    eventStore_toneoff{n}.vertChange   = vertChange;
    eventStore_toneoff{n}.change       = change;
    eventStore_toneoff{n}.trigger      = tone(n);
    eventStore_toneoff{n}.trigger_type = 'tone';
    ...
    for m = 25:101
        ...
        if ((change(m) > (nanmean(change(1:20)) + (4*nanstd(change(1:20))))) || ...
            ...
            if ~found
                eventStore_toneoff{n}.twitchFrame = stuff(m,n) + 1;
                found = 1;
            end
        end
    end
    ...
    if ~found
        eventStore_toneoff{n}.twitchFrame = NaN;
        eventStore_toneoff{n}.twitch_x    = NaN;
        eventStore_toneoff{n}.twitch_y    = NaN;
    else
        disp(['Twitch found in file: img' num2str(eventStore_toneoff{n}.twitchFrame) '.tif']);
        
        % Get the maximum change location as follows...
        
        % Take twitch frame...
        twitchy = double(imread(['img' num2str(eventStore_toneoff{n}.twitchFrame) '.tif']));
        ...
        for xdir = 1:(x2 - x1 - 28)
            if sum(flat_x(xdir:(xdir+29))) > score_x;
                ...
                eventStore_toneoff{n}.twitch_x = xdir;% + 99;     % take offset into account
            end
        end
        ...
        for ydir = 1:(y2 - y1 - 28)
            if sum(flat_y(ydir:(ydir+29))) > score_y;
                ...
                eventStore_toneoff{n}.twitch_y = ydir;          % no offset
            end
        end
    end
    
    eventStore_toneoff{n}.referenceFrame = ref;
    
end

%save('twitches_toneoff_triggered.mat', 'eventStore')
